...
class Cafe extends CI_Controller
{
	...
	public function index()
	{
        ...
        if($chkMobile){
            ...
            $data = array();
            $countAll = $this->cafe_m->img_get(null);
        ...
        $img_get = $this->cafe_m->img_get_detail_list(null,$param);
    ...
    public function listAll()
    {
        ...
        $this->load->library('pagination');
        $config['base_url'] = 'http://www.restlife.shop/Cafe/listAll';
	...
	public function lists()
	{
		...
		//게시판 글 카운트  유무 확인
		$ckCountView = $this->cafe_m->ckCountView($dir,$index,$_SERVER['REMOTE_ADDR']);
		...
		$data =array();
		$img_get = $this->cafe_m->img_get($index);
		$data['img_get'] = $img_get;
		$data['password'] = isset($img_get[0]['password'])?$img_get[0]['password']:"null";
		$data['session' ] = $this->sessions;
		$likeButtonCount = $this->cafe_m->likeButtonCount($index);
		$data['likeButtonCount'] = count($likeButtonCount);
		$replyComment    = $this->cafe_m->replyComment($index);
		$CountView    = $this->cafe_m->CountView( $dir,$index );//조회수 갯수 몇개인지 체크 (2021-06-06)
		...
		$img_get_all = $this->cafe_m->img_get(null);
	...
	public function upload()
	{
		...
		$uploadfullPath = "../www/lsm/img/cafe/";
		// git,jpg,png 파일만 업로드를 허용한다.
		//$config['allowed_types'] = 'gif|jpg|png';
		$imageBaseUrl = "/lsm/img/cafe/";
		// 허용되는 파일의 최대 사이즈
		//$config['max_size'] = '100';
		// 이미지인 경우 허용되는 최대 폭
	...
	public function uploadForm()
	{
		...
		if($update != 'update'){
			...
			if(!empty($src[1])){
				...
				$index = $this->cafe_m->get_index($jpg_name);
			}
			...
			echo "<script>location.href='http://www.restlife.shop/Cafe/listAll'</script>";
